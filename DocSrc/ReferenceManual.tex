...
will be fitted as a series of 1Ds sub spectra to extract integral parameters. Note that each sub-spectrum possess its own set of parameters.
See \hyperref[sec:sideframe] {Side Frame section} for selecting rows.

`Fit all' buton behavior depends on choice among 3 radio buttons:
\begin{itemize}
\item `no cpy' radio button: Fit each sub-spectrum using its own set of parameters.
\item `cpy all' radio button: Copy current spectrum set of parameters to all other sub-spectra before fitting each sub-spectrum.
\item `inc cpy' radio button: Does incremental copy/fit of parameters. First copy current to first sub-spectrum and fit it. 
Continue with copying the just fitted parameters to next sub-spectrum before fitting it and repeat for each sub-spectrum. 
This is useful for series of spectra where the fitted parameters evolve slowly from one sub-spectrum to the next one.
\end{itemize}
If convergence is not reached after `inc cpy', then switching to `no cpy' will refine the fit of each individual spectrum.
