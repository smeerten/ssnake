...
\subsection{FWHM}
FWHM, or Full Width at Half Maximum, is a tool to calcualted the width of a peak. Selecting the tool in the menu creates a window. In this, the boundaries of the peak of interest have to be set where ssNake will search for the width of the peak. Make sure that there is only one peak in the slected window, otherwise the result might be inaccurate. The easiest way to select the regions is by left clicking in the spectrum. After valid input is given, ssNake directly calculates the FWHM in the units of the current axis.
