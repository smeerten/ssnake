...
phasing can be set, as well as the pivot point (centre point) for the first order phasing. The values
...
only zero order phasing correction, or with both zero and first order. Do note that the setting that ssNake
...
Perform a 0th order autophase on the current spectrum (and applies it on all traces). This is
identical to doing this via the Phasing window, but requires less effort for the user.

\subsection{Autophase 0+1}
Perform a 1th order autophase on the current spectrum (and applies it on all traces). This is
identical to doing this via the Phasing window, but requires less effort for the user.
...
Hypercomplex data is data that has a real and imaginary part along more than 1 dimension. For a 2D
data set that is hypercomplex, four 2D data sets are necessary to describe the data: RR, RI, IR, and
II (with R = Real, and I = Imag).  Generally in NMR, the 2D data is recorded with alternating phase,
in such a way that groups of 2 data points in the indirect dimension must be combined to recover the
four hypercomplex values. Depending on the way the data is recorded, a different procedure is
necessary to construct the hypercomplex data. All of these methods start with a alternating data set consisting
of [R1, I1, R2, I2], were R1 stands for the first recorded complex point with time 1, I1 for the second
recorded complex point with time 1. After the conversion to hypercomplex data (along a specific
dimension), ssNake then does hypercomplex processing along that dimension. Phasing along a hyper
complex axis, for example, performs a different phasing than phasing over a non-hypercomplex axis.
Let's say we have a 2D data set with hypercomplex information along D1. Phasing along D2, we will
issue a regular phasing. In the notation used above (RR, IR, RI, II), this means that we will phase
between RR$\leftrightarrow$RI and IR$\leftrightarrow$II. If we phase along D1, we must phase in the
opposite way: RR$\leftrightarrow$IR and RI$\leftrightarrow$II. Also, a Fourier transform along D1
must take RR + $i\cdot$IR as its real and imaginary part (and separately transform RI + $i\cdot$II).

In principle, there can be hypercomplex data along every dimension except one. Normally in NMR, the
direct dimension (last dimension) cannot be hyper complex. This is because it does not make sense,
as the direct real data already has its imaginary counterpart: the direct imaginary data. Hypercomplex
data can be seen as adding the imaginary part to also the indirect dimensions, to allow phasing
along this dimension.


\subsubsection{States}
States conversion takes the alternating data set and makes new hypercomplex values. For 2D
data this means that when preforming states along D1, the two complex numbers are converted as:
\begin{equation*}
  \text{RR} + i \cdot \text{RI} = \text{R}_{1} + i \cdot \text{I}_{1} \quad .
\end{equation*}
\begin{equation*}
  \text{IR} + i \cdot \text{II} = \text{R}_{2} + i \cdot \text{I}_{2} \quad .
\end{equation*}
Making two new complex numbers from the old ones. In this case R$_1$ is the real value of the first
point of the set of two, and R$_2$ the second real point (and the same for the imaginary, I).
...
same procedure as States. After the procedure, the values as odd positions are multiplied with $-1$
...
\begin{equation*}
  \text{RR} + i \cdot \text{RI} = (\text{R}_{1} +\text{R}_{2})  + i \cdot (\text{I}_{1} +
  \text{I}_2)
\end{equation*}
\begin{equation*}
  \text{IR} + i \cdot \text{II} = (\text{I}_{2} - \text{I}_1) + i \cdot (\text{R}_{1} - \text{R}_2  ) \quad .
\end{equation*}
...
\subsubsection*{ssNake implementation}
Selecting `Matrix $\rightarrow$ Sizing' creates a sizing window. In here a
single number has to be filled in the box titled `Size'. Decimal values will be
rounded to the nearest integer. Apart from numbers, the command `k' (or `K') is
also supported, which stands for `1024'. When using this definition there has
to be a number before the `k'. A valid input would therefore be: `2k'. Which
sets the number of points to $2 \cdot 1024=2048$. Pressing `Apply' executes the
sizing command on all traces in all dimensions. Also, the $\pm 2\text{\textasciicircum}n$ buttons can be used
to go to the nearest power of 2 (which makes subsequent Fourier transforms much faster).

If the supplied number is larger than the current size, zeroes are added at the
right-hand side of the time signal. If the value is lower, points are removed
from the right-hand side of the time signal. If the whole echo type is on, the
zeroes are added in the centre of the time signal, or points are taken
symmetrically from the centre, if the size is reduced. Alternatively, the
position of the zero filling can be specified by left-clicking in the spectrum,
or by manually filling in the desired offset value in the box `Offset'.

When applied to the frequency domain, zero filling makes no real sense.
Therefore, ssNake always applies it in the time domain. So when the size is
changed within a spectrum, it is transformed back to the time domain, the size
is changed, and a forward Fourier transform gets the data back to the frequency
domain.
...
A tool to scale a selected region to a particular maximum, minimum, or integral. This is
essentially the same as the `Multiply' tool, only ssNake calculates the relevant multiplication
factor for you.
...
Roughly speaking, ssNake uses two types of fitting procedures: those with a mathematical solution,
and those who have to be approximated iteratively. An example of the former type is the fitting of
the width of a peak at halve height (FWHM). This involves nothing more than finding the left and
right side of the peak (at halve height) and to calculate the width of this interval. This can be
done in one easy mathematical procedure, and involves no iterative loop. The second type of fitting
procedure is used for the powder lineshapes (e.g. quadrupolar and chemical shift lineshapes). With
these problems, there is no direct mathematical solution. We are searching a parameter space for the
solution that best fits the experimental spectrum. This is done by trying different settings, and
qualifying how well these fit. Based on these attempts, a new setting is tested. This is continued
until an optimum has been found. There are several ways to do such an iterative fitting: ssNake uses
the Nelder-Mead algorithm \cite{nelder1965simplex} (also known as ``Simplex'').
...
state NMR. In the following sections, the equations for these shapes fill be given. In all cases,
...
show the current guess, and put all variables to reasonable values.
...
As explained in the previous section, simulation of powder patterns requires averaging over a
sphere. In practice, only a finite amount of angles (the polar angles $\theta$ and $\phi$) is
...
The spinning case is somewhat more difficult. Essentially, the FID of one rotor period is simulated using $n$
steps, with $n$ the desired number of sidebands. This FID is then Fourier transformed, giving the intensities
of the sidebands. Of these, a spectrum is made, adding a shift and line broadening. The is done for all the
crystallites, using $n$ gamma angles.
...
\subsection{Czjzek}
This simulates a static or `infinite' MAS spectrum of a Czjzek distribution (half-integer spins $> 1/2$
only). A Czjzek distribution exist in materials were the local environment of a quadrupole nucleus is
distorted in a specific way.

The distribution is defined as:
\begin{equation}
  A(\omega_\text{Q},\eta) = \frac{\omega_\text{Q}{}^{d - 1} \cdot \eta}{\sqrt{2\pi} \cdot \sigma^d} (1 - \eta^2 / 9) \cdot
  \exp\left[\frac{-\omega_\text{Q}^2 (1 + \eta^2/3)} {2\sigma^2}   \right]
\end{equation}
This equation gives the amplitude $A$ for a specific $\omega_\text{Q}$, $\eta$, $\sigma$ and $d$. $d$ is the order
parameter (equal to 5 in the initially proposed model by Czjzek and co-workers) and $\sigma$ the strength of
the quadrupolar interaction (in MHz). The other parameter are the
regular quadrupolar parameters. To simulate a Czjzek spectrum, one must simulate regular 2nd order quadrupolar
spectra for quadrupolars values $0\rightarrow 1$ for $\eta$, and $0\rightarrow \infty$ for $\omega_\text{Q}$.
However, as $a$ becomes very small and essentially all values beyond $\omega_\text{Q} > 8\cdot \sigma$ can be
ignored. The maximum of $A$ is at $\omega_\text{Q} = 2\sigma$ and $\eta = 0.68$. Within ssNake, the value
$\omega_\text{Q}{}^\text{max}$ is therefore given in units of $\sigma$, and has a default value of 4.

To simulate this, ssNake creates a linear grid on the 2D surface of $\eta$ and $\omega_\text{Q}$ using a
specified number of steps along both dimensions, and a maximum value along the $\omega_\text{Q}$ axis. For all
of these values, a 2nd order quadrupolar spectrum is calculated (static or spinning). This creates a library
of spectra for each $\eta$ and $\omega_\text{Q}$ value. When fitting, ssNake only calculates this library
once.

To calculate a Czjzek spectrum, a value for $d$ and $\sigma$ need to be given. For this, the $A$ coefficients
can be calculated. Now, all the spectra in the library can be multiplied by their $A$-value, and added to give
the final spectrum.

As the library is only calculated once, the $\omega_\text{Q}{}^\text{max}$ must be appropriately set. When
$\sigma$ has changed a lot during the fit, it is wise to rerun the fit. This second fit then runs with a
better tweaked library for this specific value of $\sigma$.
...
`Simpson' tool can be used to fit spectra via the Simpson package
\cite{Bak2000SIMPSON,Tosner2014Computer,Tosner2009Optimal}. For this, a Simpson input script is
...
then recognizes the differnt varibales defined in this way. When a simulation is run, ssNake
...
If, for
example, we want to fit a spectrum were we want to change the chemical shift anisotropy
...
that the `@' sign is only used by ssNake to recognise which are variable in the equation.
