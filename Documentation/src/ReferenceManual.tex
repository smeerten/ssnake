...
\end{equation}
This equation gives the amplitude $A$ for a specific $C_\text{Q}$, $\eta$, $\sigma$ and $d$. $d$ is the order
...
spectra for quadrupolars values $0\rightarrow 1$ for $\eta$, and $0\rightarrow \infty$ for $C_\text{Q}$.
However, as $A$ becomes very small and essentially all values beyond $C_\text{Q} > 8\cdot \sigma$ can be
ignored. The maximum of $A$ is at $C_\text{Q} = 2\sigma$ and $\eta = 0.68$. Within ssNake, the value
$C_\text{Q}{}^\text{max}$ is therefore given in units of $\sigma$, and has a default value of 4.

To simulate this, ssNake creates a linear grid on the 2D surface of $\eta$ and $C_\text{Q}$ using a
specified number of steps along both dimensions, and a maximum value along the $C_\text{Q}$ axis. For all
of these values, a 2nd order quadrupolar spectrum is calculated (static or spinning). This creates a library
of spectra for each $\eta$ and $C_\text{Q}$ value. When fitting, ssNake only calculates this library
once.
...
As the library is only calculated once, the $C_\text{Q}{}^\text{max}$ must be appropriately set. When
