...
#        action4 = self.macrorenamemenu.addAction(givenName, lambda name=givenName: self.renameMacro(name))
    ...
    def dataFromFit(self, data, freq, sw, spec, wholeEcho, ref, xaxArray, axes):
        ...
        if name is None:
            return
        reloadFunc = lambda self: self.dataFromFit(data,
                                                   freq,
                                                   sw,
                                                   spec,
                                                   wholeEcho,
    ...
    def loadJSONFile(self, filePath, name=''):
        ...
        masterData = sc.Spectrum(name,
                                 data,
                                 lambda self: self.loadJSONFile(filePath),
                                 list(struct['freq']),
                                 list(struct['sw']),
                                 list(struct['spec']),
    ...
    def loadMatlabFile(self, filePath, name=''):
        ...
        if version < 7.3:  # all versions below 7.3 are supported
            ...
            masterData = sc.Spectrum(name,
                                     data,
                                     lambda self: self.loadMatlabFile(filePath),
                                     list(mat['freq'][0, 0][0]),
                                     list(mat['sw'][0, 0][0]),
                                     list(mat['spec'][0, 0][0]),
...
class TextFrame(QtGui.QScrollArea):
    ...
    def __init__(self, parent):
        ...
        grid.addWidget(wc.QLabel("Position:"), 0, 2)
        ...
        grid.addWidget(wc.QLabel("x-value:"), 0, 4)
        ...
        grid.addWidget(wc.QLabel("amp:"), 0, 8)
        ...
        grid.addWidget(wc.QLabel(u"\u0394x:"), 0, 10)
        ...
        grid.addWidget(wc.QLabel(u"\u0394amp:"), 0, 14)
